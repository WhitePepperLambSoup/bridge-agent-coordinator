...
```powershell
git tag -a v0.2.0 -m "Bridge 0.2.0"
git push origin v0.2.0
```

在 GitHub 仓库页面选择 **Releases → Draft a new release**，选择 `v0.2.0`，标题填写
`Bridge 0.2.0`，发布说明可参考 `CHANGELOG.md`。Actions 生成的 distributions artifact
...
create and push the `v0.2.0` annotated tag and draft a GitHub Release using `CHANGELOG.md`.
