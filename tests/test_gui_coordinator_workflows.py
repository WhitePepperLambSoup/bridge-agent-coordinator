...
class _FakeGuiWidget:
    ...
    def insert(self, _index, value, *_args):
        self.content += value
...
class _FakeGuiWindow(_FakeGuiWidget):
...
def _run_gui_error_callback(monkeypatch, method_name, task):
    """Run one asynchronous GUI error path synchronously with fake widgets."""
    widgets = []
    ...
    class _FailingCoordinator:
        def generate_ai_pre_review(self, _task_id):
            raise RuntimeError("pre-review exploded")

        def get_worktree(self, _task_id):
            return None

        def diagnose_task_failure(self, _task_id):
            raise RuntimeError("diagnosis exploded")

    def make_widget(*_args, **_kwargs):
        widget = _FakeGuiWidget()
        widgets.append(widget)
        return widget

    window = _FakeGuiWindow()
    monkeypatch.setattr(gui.tk, "Toplevel", lambda *_args, **_kwargs: window)
    monkeypatch.setattr(gui.ttk, "Frame", make_widget)
    monkeypatch.setattr(gui.ttk, "Label", make_widget)
    monkeypatch.setattr(gui.scrolledtext, "ScrolledText", make_widget)
    ...
    app.root = fake_root
    app.lang = "zh"
    app.coordinator = _FailingCoordinator()
    app._selected_task = lambda: task
    app._ai_review_windows = {}
    app._conflict_windows = {}

    getattr(app, method_name)()
    for callback in fake_root.callbacks:
        callback()
    assert any("exploded" in widget.content for widget in widgets)


def test_gui_ai_pre_review_error_is_rendered(monkeypatch):
    _run_gui_error_callback(
        monkeypatch,
        "_gui_ai_pre_review",
        {"id": "task-pre-review", "state": TaskState.IN_PROGRESS.value},
    )


def test_gui_conflict_diagnosis_error_is_rendered(monkeypatch):
    _run_gui_error_callback(
        monkeypatch,
        "_gui_ai_conflict_diagnosis",
        {"id": "task-diagnosis", "state": TaskState.IN_PROGRESS.value},
    )
