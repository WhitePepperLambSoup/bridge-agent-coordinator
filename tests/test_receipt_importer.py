...
class TestReceiptImport:
    ...
    def test_concurrent_duplicate_import_is_idempotent_and_releases_transactions(
        self, task_dir, db, ids
    ):
        """Two database connections importing one receipt must not poison SQLite."""
        real_commit = db[1]
        _write_receipt(
            task_dir,
            task_id=ids["task_id"],
        ...
        second_db = init_database(db[0].path)
        importers = [
            ReceiptImporter(db[0], stability_ms=0),
            ReceiptImporter(second_db, stability_ms=0),
        ]
        for current in importers:
            current._wait_stable = lambda _path: True
        ...
        def run(current):
            barrier.wait()
            try:
                result = current.scan_and_import(
                    task_dir, ids["task_id"], 1, "lease-abc", ids["agent_id"]
                )
                outputs.append(("ok", result))
            except Exception as exc:  # pragma: no cover - assertion reports the type
                outputs.append(("err", type(exc).__name__, str(exc)))

        threads = [threading.Thread(target=run, args=(current,)) for current in importers]
        ...
        assert all(item[0] == "ok" for item in outputs), outputs
        assert db[0].conn.execute("SELECT COUNT(*) FROM receipts").fetchone()[0] == 1

        second_db.conn.execute("SELECT 1").fetchone()
        second_db.close()
