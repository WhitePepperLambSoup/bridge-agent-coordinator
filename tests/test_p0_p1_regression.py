...
class TestP1AttemptLifecycle:
    ...
    def test_done_transition_closes_attempt_and_lease(self, coordinator, env):
        """A completed task must not leave an active attempt or lease behind."""
        tid = _prepare_approved(coordinator, env)
        lease = coordinator.db.list_leases_by_task(tid)[0]
        attempt = coordinator.get_current_attempt(tid)
        assert attempt is not None
        assert lease["attempt_id"] == attempt["id"]

        entry = coordinator.enqueue_merge(tid, "candidate", confirmed=True)
        coordinator.transition_task(tid, TaskState.MERGE_QUEUED, confirmed=True)
        coordinator.start_merge(entry.entry_id, confirmed=True)
        coordinator.transition_task(tid, TaskState.MERGING, confirmed=True)
        coordinator.complete_merge(entry.entry_id, "merged", confirmed=True)
        coordinator.transition_task(tid, TaskState.DONE, confirmed=True)

        assert coordinator.get_current_attempt(tid) is None
        assert coordinator.db.get_lease(lease["id"])["status"] == "revoked"
        assert coordinator.leases.get(lease["id"]).status == "revoked"
