...
class TestP0MergeNoWorkspacePollution:
    ...
    def test_cherry_pick_conflict_remains_conflict_in_memory_and_database(self, coordinator, git_env):
        pid, impl, rev, gid, tid, repo = git_env
        conflict_file = os.path.join(repo, "conflict.txt")
        with open(conflict_file, "w", encoding="utf-8") as handle:
            handle.write("base\n")
        subprocess.run(["git", "add", "conflict.txt"], cwd=repo, check=True)
        subprocess.run(["git", "commit", "-m", "add conflict file"], cwd=repo, check=True)

        subprocess.run(["git", "checkout", "-b", "conflict-feature"], cwd=repo, check=True)
        with open(conflict_file, "w", encoding="utf-8") as handle:
        ...
        _prepare_approved(coordinator, git_env)
        entry = coordinator.enqueue_merge(tid, feature_commit, confirmed=True)
        with pytest.raises(CoordinatorError, match="conflict|Conflict"):
            coordinator.start_merge(entry.entry_id, confirmed=True)
        assert coordinator.get_merge_entry(entry.entry_id).status == MergeStatus.CONFLICT
        assert coordinator.merge.get(entry.entry_id).status == MergeStatus.CONFLICT
