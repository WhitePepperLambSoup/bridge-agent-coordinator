...
class BridgeApp:
    ...
    def _route_mousewheel(self, event):
        ...
        for page in self.scroll_pages:
            try:
                if not page.contains_pointer() or not page.owns_widget(event.widget):
                    continue
                if page.has_nested_scroll_target(event.widget):
                    return None
    ...
    def _build_coordinator_tab(self, tab):
        ...
        self.task_meta_label.pack(anchor=tk.W, pady=(4, 14))
    ...
    def _on_task_select(self, _event=None):
        ...
        self.task_meta_label.config(
            text=f"{task['id']}  ·  {STATE_LABELS.get(state, state)}  ·  风险 {task.get('risk', 'medium')}"
        )
    ...
    def _transition_with_confirmation(self, task_id: str, state: TaskState, actor="user") -> bool:
        try:
            self.coordinator.transition_task(task_id, state, actor=actor, confirmed=False)
            return True
            ...
            if not messagebox.askyesno(
                ...
                return False
            self.coordinator.transition_task(task_id, state, actor=actor, confirmed=True)
            return True
    ...
    def _prepare_selected_task(self):
        ...
        try:
            if task["state"] == TaskState.DRAFT.value:
                self.coordinator.transition_task(task["id"], TaskState.PLANNING, actor="user")
    ...
    def _build_implementer_prompt(self, task: dict) -> str:
        ...
        owner = self.coordinator.get_agent(task.get("owner_agent_id", "")) or {}
        return (
    ...
    def _revoke_attempt_lease(self, attempt: dict, reason: str):
        ...
        if not lease_id:
            return
        try:
            self.coordinator.leases.revoke(lease_id, reason=reason)
        except Exception:
            pass
        self.coordinator.db.update_lease_status(lease_id, "revoked", reason)
    ...
    def _gui_validate_and_review(self):
        ...
        try:
            ...
            if failed:
                raise CoordinatorError(
                    "Validation failed: "
                    + ", ".join(r.get("check_id", "unknown") for r in failed)
                )
    ...
    def _cleanup_completed_task(self, task_id: str):
        ...
        if attempt:
            self.coordinator.complete_attempt(attempt["id"], status="completed")
            self._revoke_attempt_lease(attempt, "task completed")
        if self.coordinator.get_worktree(task_id):
            try:
                self.coordinator.remove_worktree(task_id, force=False, confirmed=True)
            except Exception as error:
