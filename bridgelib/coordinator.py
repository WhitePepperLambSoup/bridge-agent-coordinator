...
from bridgelib.ai_assistant import AIAssistant, AIPreReviewReport, AutoFixDiagnosis
...
class BridgeCoordinator:
    ...
    def __init__(
        ...
        self._receipt_watcher: Any = None
        self._receipt_watcher_db: Database | None = None
    ...
    def close(self):
        """Stop background resources and close the authoritative database."""
        self.stop_receipt_watcher()
        try:
    ...
    def transition_task(self, task_id: str, to_state: TaskState, actor: str = "system",
        ...
        transition_result = self.db.update_task_state_with_event(
            task_id=task_id,
            ...
            close_active_resources=(to_state == TaskState.DONE),
            resource_reason="task completed" if to_state == TaskState.DONE else "",
        )
        if to_state == TaskState.DONE:
            # SQLite is authoritative, but keep the in-memory lease manager in
            # sync so subsequent callers cannot acquire a stale conflicting lease.
            for lease_id in transition_result.get("lease_ids", []):
                lease = self.leases.get(lease_id)
                if lease is not None and lease.status == "active":
                    try:
                        self.leases.revoke(lease_id, reason="task completed")
                    except Exception:
                        # The database transition already succeeded.  A missing
                        # in-memory lease should not turn a successful DONE into
                        # a false failure.
                        pass
        if to_state in (TaskState.REVISION_REQUIRED, TaskState.BLOCKED, TaskState.CONFLICT):
            try:
                self.retry.record_failure(task_id, f"Transitioned to {to_state.value}")
            except Exception:
                pass
        if to_state == TaskState.DONE:
            try:
                children = self.db.get_dependent_tasks(task_id)
                for child_id in children:
                    met, _ = self.check_task_dependencies_met(child_id)
                    if met:
                        self.db.write_event(
                            event_type="TaskDependenciesSatisfied",
                            actor_type="system",
                            actor_id="coordinator",
                            task_id=child_id,
                            project_id=task.get("project_id", ""),
                            payload={"unlocked_task_id": child_id, "satisfied_by": task_id},
                        )
            except Exception:
                pass
    ...
    def get_active_lease(self, task_id: str) -> dict | None:
        """Return active lease record for a task if one exists and has not expired."""
        from datetime import datetime, timezone as tz
        now_utc = datetime.now(tz.utc).isoformat()
        leases = self.db.list_leases_by_task(task_id)
        for l in leases:
    ...
    def start_merge(self, entry_id: str, confirmed: bool = False) -> MergeEntry:
        ...
        try:
            ...
            try:
                ...
                pass
            # Preserve a real conflict so the GUI/recovery flow can resolve it;
            # only ordinary failures are safe to requeue automatically.
    ...
    def _try_git_cherry_pick(self, entry: MergeEntry, project_id: str):
        ...
        r_check = adapter._run(["git", "rev-parse", "--verify", target_branch])
        if r_check.returncode != 0:
            if target_branch == "main":
                r_master = adapter._run(["git", "rev-parse", "--verify", "master"])
                if r_master.returncode == 0:
                    target_branch = "master"
            elif target_branch == "master":
                r_main = adapter._run(["git", "rev-parse", "--verify", "main"])
    ...
    def _complete_git_merge(self, entry: MergeEntry, project_id: str):
        ...
        try:
            ...
            if checked_out_wt_path:
                # Verify that the checked out worktree has no uncommitted changes before merging.
                r_dirty = adapter._run(["git", "status", "--porcelain"], cwd=checked_out_wt_path)
                if r_dirty.returncode == 0 and r_dirty.stdout.strip():
                    lines = [l for l in r_dirty.stdout.strip().split("\n") if not l.startswith("??")]
                    if lines:
                        raise CoordinatorError(
                            f"Target branch worktree '{checked_out_wt_path}' has uncommitted changes. "
                            f"Please commit or stash them before completing the merge."
                        )
    ...
    def get_project_summary(self, project_id: str) -> ProjectSummary:
        ...
        return ProjectSummary(
            ...
            queued_merges=sum(
                1 for row in self.db.list_merge_entries(project_id=project_id)
                if row.get("status") == MergeStatus.QUEUED
            ),
        )
    ...
    def record_task_cost(self, task_id: str, agent_id: str,
        ...
        if isinstance(input_tokens, bool) or isinstance(output_tokens, bool):
            raise CoordinatorError("Token counts must be non-negative integers")
        if input_tokens < 0 or output_tokens < 0:
            raise CoordinatorError("Token counts must be non-negative")
        from datetime import datetime, timezone
        record = self.costs.record(
            task_id, agent_id, input_tokens, output_tokens,
            estimated_cost, is_estimated=is_estimated, source=source,
        )
        try:
            self.db.conn.execute(
                ...
                (f"cost-{task_id}-{agent_id}-{datetime.now(timezone.utc).timestamp()}",
                 task_id, agent_id, input_tokens, output_tokens,
                 estimated_cost, int(is_estimated), source, datetime.now(timezone.utc).isoformat()),
            )
            self.db.conn.commit()
        except Exception:
            self.costs.discard(record)
            raise
    ...
    def get_git_adapter(self) -> object:
        ...
        from bridgelib.git_adapter import GitRepositoryAdapter
        project = self.db.get_project(self._active_project_id or "")
        root_path = project.get("root_path", ".") if project else "."
        return GitRepositoryAdapter(root_path)
    ...
    def _cross_validate_git_diff(self, project_id: str, base_commit: str,
        ...
        try:
            ...
            proj = self.db.get_project(project_id)
            if not proj:
                return [f"Cannot cross-validate Git diff: project '{project_id}' not found"]
            adapter = GitRepositoryAdapter(proj["root_path"])
            ...
            issues.append(f"Cannot cross-validate Git diff: {exc}")
    ...
    def import_receipt(self, task_dir: str, task_id: str, attempt: int,
                       lease_id: str, agent_id: str, confirmed: bool = False,
                       record_estimated_cost: bool = True) -> dict | None:
        ...
        res = importer.scan_and_import(
            task_dir, task_id, attempt, lease_id, agent_id,
            allowed_paths=allowed, forbidden_paths=forbidden,
        )
        if res and record_estimated_cost:
            # Auto-record token cost tracking metrics
            # Receipt metadata currently has no token fields, so these are
            # explicitly estimated defaults.  Do not silently swallow a
            # persistence error: an accepted receipt must have an auditable
            # cost record, otherwise budget reporting becomes misleading.
            tokens_in = int(res.get("input_tokens", 0) or 2500)
            tokens_out = int(res.get("output_tokens", 0) or 800)
            self.record_task_cost(
                task_id, agent_id,
            ...
            if res.get("status") == "blocked":
                self.record_task_failure(task_id, f"Agent reported blocked: {res.get('receipt_id')}")
        return res
    ...
    def create_worktree(self, task_id: str, agent_id: str, attempt: int = 1,
                        base_commit: str = "", confirmed: bool = False,
                        inherit_config: bool = False) -> dict:
        ...
        ws = self.workspaces.register(
            ...
            base_commit=base_commit, project_id=project_id,
        )
            ...
            if proj and os.path.isdir(proj.get("root_path", "")):
                ...
                try:
                    ...
                    if repo_status.is_repo:
                        ...
                        ws_dir_name = workspace_dir_name(
                            agent_id, task_id, project_id=project_id,
                        )
                        worktree_path = os.path.join(
                            root_path, "..", ".bridge-task-worktrees", ws_dir_name
                        )
                        # 1. Resolve actual base commit SHA
                        actual_base = base_commit
                        if not actual_base:
                            r_head = adapter._run(["git", "rev-parse", "HEAD"])
                            if r_head.returncode == 0 and r_head.stdout.strip():
                                actual_base = r_head.stdout.strip()
                            else:
                                actual_base = "HEAD"
                        base_commit = actual_base
                        ws.base_commit = actual_base

                        # Create the task branch first.
                        r = adapter._run([
                            "git", "branch", ws.branch, actual_base
                        ])
                        if r.returncode != 0:
                            raise CoordinatorError(
                        ...
                        # 4. Workspace Hygiene: inherit local environment configs from root_path if present
                        config_candidates = [".env.example"]
                        if inherit_config:
                        ...
                        for cfg_name in config_candidates:
                            src_cfg = os.path.join(root_path, cfg_name)
                            dst_cfg = os.path.join(worktree_path, cfg_name)
                            if os.path.isfile(src_cfg) and not os.path.exists(dst_cfg):
                                try:
                                    shutil.copy2(src_cfg, dst_cfg)
                                except Exception as copy_err:
                                    logger.debug(f"Could not inherit {cfg_name} to worktree: {copy_err}")
                    else:
                        raise CoordinatorError(
                            f"Project root '{root_path}' is not a Git repository"
                        )
    ...
    def create_attempt(self, task_id: str, agent_id: str,
        ...
        if lease_id:
            updated = self.db.conn.execute(
                "UPDATE leases SET attempt_id = ? "
                "WHERE id = ? AND task_id = ? AND agent_id = ?",
                (attempt_id, lease_id, task_id, agent_id),
            )
            if updated.rowcount != 1:
                self.db.conn.rollback()
                raise CoordinatorError(
                    f"Lease {lease_id} could not be linked to attempt {attempt_id}"
                )
        self.db.conn.commit()

        lease_obj = self.leases.get(lease_id) if lease_id else None
        if lease_obj is not None:
            lease_obj.attempt_id = attempt_id
    ...
    def start_receipt_watcher(self, task_dir: str, task_id: str, attempt: int,
        ...
        if self._receipt_watcher is not None:
            self.stop_receipt_watcher()
        self._receipt_watcher = ReceiptWatcher(importer)
        self._receipt_watcher_db = thread_db
        if on_imported:
            self._receipt_watcher.on_imported(on_imported)
    ...
    def stop_receipt_watcher(self):
        ...
        if self._receipt_watcher_db is not None:
            try:
                self._receipt_watcher_db.close()
            finally:
                self._receipt_watcher_db = None
        self._receipt_watcher = None
    ...
    # ── AI Assistant & Self-Healing Loop ─────────────────

    def generate_ai_pre_review(self, task_id: str) -> AIPreReviewReport:
        """Generate an AI pre-review report analyzing code diff against criteria and safety."""
        task = self.db.get_task(task_id)
        if task is None:
            raise CoordinatorError(f"Task {task_id} not found")
        ...
        ws = self.get_worktree(task_id)
        worktree_path = ws.get("worktree_path", "") if ws else ""
        base_commit = (ws.get("base_commit", "") if ws else "") or "HEAD~1"
        ...
        if proj and os.path.isdir(proj.get("root_path", "")):
            try:
                ...
                if worktree_path and os.path.isdir(worktree_path):
                    r_diff = adapter._run(["git", "diff", base_commit], cwd=worktree_path)
                    if r_diff.returncode == 0:
                        diff_text = r_diff.stdout
                    r_names = adapter._run(["git", "diff", "--name-only", base_commit], cwd=worktree_path)
                    if r_names.returncode == 0:
                        changed_files = [f.strip() for f in r_names.stdout.splitlines() if f.strip()]
            except Exception:
                pass

        # Load latest receipt if present
        receipt_dict = None
        receipt_row = self.db.conn.execute(
            "SELECT * FROM receipts WHERE task_id = ? ORDER BY created_at DESC LIMIT 1",
            (task_id,),
        ).fetchone()
        if receipt_row:
            receipt_dict = dict(receipt_row)

        report = self.ai_assistant.pre_review(
            task=task,
            diff_text=diff_text,
        ...
        # Audit event
        self.db.write_event(
            event_type="AIPreReviewGenerated",
            actor_type="system",
    ...
    def diagnose_task_failure(self, task_id: str) -> AutoFixDiagnosis:
        """Diagnose validation failures and provide remediation prompt and patch."""
        task = self.db.get_task(task_id)
        if task is None:
            raise CoordinatorError(f"Task {task_id} not found")

        validations = [dict(v) for v in self.db.list_validations_by_task(task_id)]
        diff_text = ""
        ws = self.get_worktree(task_id)
        worktree_path = ws.get("worktree_path", "") if ws else ""
        base_commit = (ws.get("base_commit", "") if ws else "") or "HEAD~1"
        ...
        diagnosis = self.ai_assistant.diagnose_failure(
            task=task,
            validation_results=validations,
            diff_text=diff_text,
        )

        # Record failure into retry manager if not already recorded
        if diagnosis.root_cause:
            try:
                self.record_task_failure(task_id, diagnosis.root_cause)
            except Exception:
                pass

        self.db.write_event(
            event_type="AutoFixDiagnosisGenerated",
            actor_type="system",
            actor_id="ai_assistant",
            task_id=task_id,
            project_id=project_id,
    ...
    def diagnose_merge_conflict(self, task_id: str, conflict_files: list[str],
                                conflict_diff: str = "") -> dict:
        """Diagnose 3-way merge conflict and provide resolution instructions."""
        task = self.db.get_task(task_id)
        res = self.ai_assistant.diagnose_merge_conflict(
            task_id=task_id,
            conflict_files=conflict_files,
            conflict_diff=conflict_diff,
        )
        self.db.write_event(
            event_type="MergeConflictDiagnosed",
            actor_type="system",
            actor_id="ai_assistant",
            task_id=task_id,
            project_id=task.get("project_id", "") if task else "",
    ...
    # ── Task Dependency DAG & Diff Viewer ────────────────

    def add_task_dependency(self, task_id: str, depends_on_task_id: str) -> bool:
        """Add a dependency relationship after cycle checking."""
        if task_id == depends_on_task_id:
            raise CoordinatorError("A task cannot depend on itself")
        t1 = self.db.get_task(task_id)
        if not t1:
            raise CoordinatorError(f"Task '{task_id}' not found")
        t2 = self.db.get_task(depends_on_task_id)
        if not t2:
            raise CoordinatorError(f"Dependency task '{depends_on_task_id}' not found")
        if t1.get("project_id") != t2.get("project_id"):
            raise CoordinatorError(
                "Tasks from different projects cannot have a dependency relationship"
            )

        # Cycle check: check if task_id is reachable from depends_on_task_id
        visited = set()
        queue = [depends_on_task_id]
        while queue:
            curr = queue.pop(0)
            if curr == task_id:
                raise CoordinatorError(f"Circular dependency detected between '{task_id}' and '{depends_on_task_id}'")
            if curr not in visited:
                visited.add(curr)
                queue.extend(self.db.get_task_dependencies(curr))

        res = self.db.add_task_dependency(task_id, depends_on_task_id)
        self.db.write_event(
            event_type="TaskDependencyAdded",
            actor_type="system",
            actor_id="coordinator",
            task_id=task_id,
            project_id=t1.get("project_id", ""),
    ...
    def remove_task_dependency(self, task_id: str, depends_on_task_id: str) -> bool:
        ...
        return self.db.remove_task_dependency(task_id, depends_on_task_id)

    def check_task_dependencies_met(self, task_id: str) -> tuple[bool, list[str]]:
        """Check if all parent dependencies of this task are in DONE state."""
        status = self.get_task_dependency_status(task_id)
        return (status["met"], status["unmet_dependencies"])

    def get_task_dependency_status(self, task_id: str) -> dict:
        """Detailed inspection of task dependencies including blocked/cancelled states."""
        parents = self.db.get_task_dependencies(task_id)
        parent_details = []
        unmet = []
        has_failed = False
        has_cancelled = False
        ...
        for pid in parents:
            ptask = self.db.get_task(pid)
            pstate = ptask.get("state") if ptask else "missing"
            ptitle = ptask.get("title") if ptask else "Unknown"
            is_done = (pstate == TaskState.DONE.value)
            if not is_done:
                unmet.append(pid)
                if pstate == TaskState.CANCELLED.value:
                    has_cancelled = True
                elif pstate in (TaskState.BLOCKED.value, TaskState.CONFLICT.value):
                    has_blocked = True
                elif pstate in (TaskState.REVISION_REQUIRED.value, "failed"):
                    has_failed = True
        ...
        return {
            ...
            "has_failed_parent": has_failed,
            "has_cancelled_parent": has_cancelled,
            "has_blocked_parent": has_blocked,
            "permanently_blocked": has_cancelled,
        }

    def get_dag_execution_plan(self, project_id: str) -> list[list[str]]:
        """Calculate parallel execution stages / layers using topological ordering."""
        tasks = self.db.get_project_tasks(project_id)
        if not tasks:
            return []
        task_ids = {t["id"] for t in tasks}
        deps = {tid: set(self.db.get_task_dependencies(tid)) & task_ids for tid in task_ids}

        completed = set()
        for t in tasks:
            if t.get("state") == TaskState.DONE.value:
                completed.add(t["id"])
        ...
        while remaining:
            ...
            if not ready_in_layer:
                # Unresolvable cycle or deadlock; append remaining tasks
                layers.append(sorted(list(remaining)))
                break
            layers.append(sorted(ready_in_layer))
            completed.update(ready_in_layer)
            remaining -= set(ready_in_layer)

        return layers

    def get_task_diff(self, task_id: str) -> dict:
        """Extract full Git diff and file modifications for a task safely without polluting root repo."""
        task = self.db.get_task(task_id)
        if not task:
            raise CoordinatorError(f"Task '{task_id}' not found")
        project_id = task.get("project_id", "")
        proj = self.db.get_project(project_id) if project_id else None
        if not proj:
            return {"task_id": task_id, "diff_text": "", "files_changed": []}

        root_path = proj.get("root_path", "")
        ws = self.get_worktree(task_id)
        worktree_path = ws.get("worktree_path", "") if ws else ""
        base_commit = (ws.get("base_commit", "") if ws else "") or "HEAD"

        # Check for committed attempt or artifacts or receipts
        submission_commit = ""
        try:
            entries = self.db.list_merge_entries(task_id=task_id)
            for ent in reversed(entries):
                c = ent.get("candidate_commit") or ent.get("commit_hash")
                if c:
                    submission_commit = c
                    break
        except Exception:
        ...
        if not submission_commit:
            try:
                receipts = self.db.list_receipts(task_id=task_id)
                for rec in reversed(receipts):
                    p = json.loads(rec.get("parsed_json") or "{}")
                    c = p.get("submission_commit") or p.get("commit") or p.get("commit_hash")
                    if c:
                        submission_commit = c
                        break
            except Exception:
        ...
        if os.path.isdir(root_path):
            try:
                ...
                if worktree_path and os.path.isdir(worktree_path):
                    # Active worktree: diff against worktree working directory
                    r_diff = adapter._run(["git", "diff", base_commit], cwd=worktree_path)
                    if r_diff.returncode == 0:
                        diff_text = r_diff.stdout
                    r_names = adapter._run(["git", "diff", "--name-only", base_commit], cwd=worktree_path)
                    if r_names.returncode == 0:
                        files_changed = [f.strip() for f in r_names.stdout.splitlines() if f.strip()]
                    r_stat = adapter._run(["git", "diff", "--shortstat", base_commit], cwd=worktree_path)
                    if r_stat.returncode == 0 and r_stat.stdout.strip():
                        diff_stat = r_stat.stdout.strip()
                elif submission_commit:
                    # Cleaned or completed worktree: diff range base_commit..submission_commit in main repo
                    target_range = f"{base_commit}..{submission_commit}"
                    r_diff = adapter._run(["git", "diff", target_range], cwd=root_path)
                    if r_diff.returncode == 0:
                        diff_text = r_diff.stdout
                    r_names = adapter._run(["git", "diff", "--name-only", target_range], cwd=root_path)
                    if r_names.returncode == 0:
                        files_changed = [f.strip() for f in r_names.stdout.splitlines() if f.strip()]
                    r_stat = adapter._run(["git", "diff", "--shortstat", target_range], cwd=root_path)
                    if r_stat.returncode == 0 and r_stat.stdout.strip():
                        diff_stat = r_stat.stdout.strip()
                else:
                    # Single-workspace mode: diff against root_path working directory
                    r_diff = adapter._run(["git", "diff", base_commit], cwd=root_path)
                    if r_diff.returncode == 0:
                        diff_text = r_diff.stdout
                    r_names = adapter._run(["git", "diff", "--name-only", base_commit], cwd=root_path)
                    if r_names.returncode == 0:
                        files_changed = [f.strip() for f in r_names.stdout.splitlines() if f.strip()]
                    r_stat = adapter._run(["git", "diff", "--shortstat", base_commit], cwd=root_path)
                    if r_stat.returncode == 0 and r_stat.stdout.strip():
                        diff_stat = r_stat.stdout.strip()
            except Exception as e:
                diff_stat = f"Diff unavailable: {e}"
